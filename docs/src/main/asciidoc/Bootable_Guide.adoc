...
Developing an application packaged as a bootable JAR is not different from developing an application for WildFly server using Maven.
...
This is done by adding extra build step to your application deployment Maven pom.xml file.
...
the server features that each layer brings. Make sure that the API and server features you are using (eg: JAX-RS, Eclipse MicroProfile Config, datasources) are brought by the 
layers you are choosing.

If you decide to not specify Galleon layers, a server containing all MicroProfile subsystems is 
