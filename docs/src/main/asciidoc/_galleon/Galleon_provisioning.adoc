...
As opposed to a WildFly zip installation that installs it all (all default 
...
Using Galleon CLI tool, you can provision WildFly installations based on a set of layers.

[NOTE]
====
In order to exclude an optional layer prefix its name with '-', for example: '-jpa' to exclude jpa layer.
====
...
==== Installation of a cloud-server and ejb with ejb-dist-cache as alternative to ejb-local-cache
